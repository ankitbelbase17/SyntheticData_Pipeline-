"""Kimurakami Gallery Scraper - Google Colab Version with S3 Support
Downloads product gallery images from kimurakami.com with cloud storage options

SETUP FOR GOOGLE COLAB:
1. Run the first cell to install dependencies:
   !pip install google-colab-selenium pillow requests boto3

2. Set up AWS credentials (if using S3):
   - Set environment variables: AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
   - Or pass them directly to the scraper
...
3. Run the scraper:
   ...
   scraper.init_driver()
   scraper.scrape_sale_page("https://kimurakami.com/collections/japanese-kimono-dress")
"""
...
class KimurakamiGalleryScraperColab:
    def __init__(
        self,
        output_dir=None,
        use_s3=False,
        ...
        """
        Initialize Kimurakami scraper for Google Colab with S3/GDrive options

        Args:
            output_dir: Local directory for temporary storage
            use_s3: Whether to upload to AWS S3
            ...
            use_google_drive: Whether to save to Google Drive (if S3 is disabled)
        """
        # S3 Configuration
        self.use_s3 = use_s3
        ...
        if self.use_s3:
            if not BOTO3_AVAILABLE:
                ...
                logger.error("=" * 60)
                logger.warning("Falling back to local/Google Drive storage.")
                self.use_s3 = False
        ...
        # Google Drive setting (only if S3 is not enabled)
        self.use_google_drive = (not self.use_s3) and use_google_drive and IN_COLAB
        ...
        if output_dir:
            ...
            self.output_dir = Path("/content/drive/MyDrive/kimurakami_dataset")
        else:
            self.output_dir = Path("/content/kimurakami_dataset")
        ...
        if self.use_s3:
            ...
            logger.info(f"Storage: Google Drive")
        else:
            logger.info(f"Storage: Local (Colab runtime)")
    ...
    def _init_s3_client(self, aws_access_key_id=None, aws_secret_access_key=None):
        ...
        try:
            ...
            logger.info(f"  Access Key provided: {'Yes' if access_key else 'No (checking env/default chain)'}")
            logger.info(f"  Secret Key provided: {'Yes' if secret_key else 'No (checking env/default chain)'}")
            ...
            if access_key and secret_key:
                ...
                # Try default credential chain (IAM role, ~/.aws/credentials, etc.)
                self.s3_client = boto3.client(
                    's3',
                    region_name=self.aws_region
                )
        ...
        try:
            if data is None:
                data = {
                    ...
                    "storage_mode": "s3" if self.use_s3 else ("google_drive" if self.use_google_drive else "local")
                }
    ...
    def init_driver(self):
        ...
        logger.info("Initializing Chrome WebDriver for Colab...")

        if IN_COLAB:
            try:
                import google_colab_selenium as gs
                self.driver = gs.Chrome()
                logger.info("Using google-colab-selenium driver")
            except ImportError:
                logger.error("google-colab-selenium not installed!")
                logger.error("Run: !pip install google-colab-selenium")
                raise ImportError("Please install google-colab-selenium: !pip install google-colab-selenium")
        else:
            ...
            try:
                ...
                if success:
                    ...
                    if self.use_s3:
                        ...
                        image_info["storage"] = "google_drive" if self.use_google_drive else "local"
                        ...
                        if product_data and len(product_data["images"]) >= 1:
                            ...
                            if len(downloaded) >= 1:
                                metadata = {
                                    ...
                                    "source": "kimurakami_gallery_colab",
                                    ...
                                    "storage": "s3" if self.use_s3 else ("google_drive" if self.use_google_drive else "local")
                                }
    ...
    def _print_final_summary(self, items_this_run):
        ...
        storage_mode = "AWS S3" if self.use_s3 else ("Google Drive" if self.use_google_drive else "Local")
...
def main():
    ...
    scraper = KimurakamiGalleryScraperColab(use_google_drive=True)

    # Option 3: Save locally (will be lost when runtime disconnects)
    scraper = KimurakamiGalleryScraperColab()

    # Run the scraper
    scraper.init_driver()
    scraper.scrape_sale_page("https://kimurakami.com/collections/japanese-kimono-dress", max_pages=2, max_items=10)
    scraper.close()
    ```
    """
    logger.info("=" * 80)
    logger.info("KIMURAKAMI GALLERY SCRAPER - GOOGLE COLAB VERSION")
    logger.info("Downloads product gallery images from kimurakami.com")
    logger.info("Supports AWS S3 and Google Drive storage")
    logger.info("=" * 80)
...
def run_scraper(
    ...
    Args:
        ...
        use_google_drive: Whether to save to Google Drive
